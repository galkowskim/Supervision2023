...
def create_url_list(n=1000):
    ...
    while offers_scraped <= offers_to_scrape:
        ...
        page_offers = soup.find_all('ul', class_='list normal')[
            0].find_all('article', class_='element')
...
def create_df(n=1000):
    ...
    df = pd.DataFrame(columns=['title', 'desc', 'user_registration_date',
                      'post_creation', 'user_offers', 'lat', 'lon', 'city', 'url'])
    ...
    for offer_url in page_urls:
        try:
            ...
            active_since = soup.find(
                'span', class_='active-since').get_text().split('od ')[1]

            user_offers = soup.find(
                'a', class_='user-offers-link').find('span').get_text()
            ...
            geo_str = soup.find('ul', class_='offerAdditionalInfo').find(
                'span').attrs['data-coordinates']
    ...
    df['user_registration_date'] = df['user_registration_date'].apply(
        user_registration_date_to_datetime)
